...
import { getRelativeTime, getCategoryBadgeColors, getStatusBadgeColors } from './CommunityFeed';
...
export const IssueDetailPage: React.FC<IssueDetailPageProps> = ({ issue, onBack }) => {
  const { upvoteIssue, updateIssueStatus, user } = useApp();
  ...
  const statusLevels: { status: IssueStatus; label: string; description: string }[] = [
    { status: 'Reported', label: 'Reported', description: 'AI triaged & cataloged.' },
    { status: 'Acknowledged', label: 'Acknowledged', description: 'Under review by municipal staff.' },
    { status: 'In Progress', label: 'In Progress', description: 'Work crew is dispatched to repair.' },
    { status: 'Resolved', label: 'Resolved', description: 'Fix completed. Hazard removed!' },
  ];

  // Find the index of current issue status
  const currentStatusIndex = statusLevels.findIndex(s => s.status === issue.status);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-md uppercase border ${getCategoryBadgeColors(issue.category)}`}>
            {issue.category}
          </span>
          <span className={`text-[10px] font-mono font-bold px-2.5 py-1 rounded-md shadow-sm ${getStatusBadgeColors(issue.status)}`}>
            {issue.status}
          </span>
        </div>
      </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-sm">
            <div className="relative bg-slate-900 max-h-[380px] overflow-hidden flex items-center justify-center">
              ...
              <div className="absolute top-4 left-4">
                <span className={`text-xs font-bold px-3 py-1 rounded-lg border shadow-md text-white ${
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                ...
                <p className="text-xs text-slate-700 italic leading-relaxed font-sans">
                  "{issue.userNote || 'The reporting citizen did not supply secondary notes.'}"
                </p>
              </div>
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                >
                  ...
                  <span>{issue.upvotes} {hasUpvoted ? 'Upvotes registered' : 'Verify (I see this too)'}</span>
                </button>
            ...
            >
              <div className="flex items-center gap-2 border-b border-slate-50 pb-2.5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400">Simulate updates from public works personnel</p>
                </div>
              </div>
              ...
              <form onSubmit={handleUpdateStatus} className="space-y-4">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">
                    Authority Worklog Update Note <span className="text-rose-500">*</span>
                  </label>
                  <textarea
                    value={authorityNote}
              ...
              {statusLevels.map((lvl, index) => {
                ...
                return (
                  <div key={lvl.status} className="relative flex gap-3.5">
                    {/* Node Circle */}
                    <div className={`absolute -left-[23px] top-1 w-[20px] h-[20px] rounded-full border-2 flex items-center justify-center z-10 transition-colors duration-300 ${
                      isPassed 
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className={`text-xs font-bold leading-none ${isCurrent ? 'text-emerald-600 font-extrabold' : isPassed ? 'text-slate-800' : 'text-slate-400'}`}>
                        {lvl.label}
                      </h4>
                      <p className="text-[10px] text-slate-400 mt-1">{lvl.description}</p>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-5">
            ...
            {issue.history && issue.history.length > 0 ? (
              <div className="space-y-4">
                {issue.history.slice().sort((a,b) => b.timestamp - a.timestamp).map((event, idx) => (
                  <div 
                    key={idx} 
                    className={`p-4 rounded-2xl border ${
                      event.status === 'Resolved' ? 'bg-emerald-50/50 border-emerald-100' :
                      event.status === 'In Progress' ? 'bg-indigo-50/50 border-indigo-100' :
                      'bg-slate-50/70 border-slate-100'
                    } space-y-2`}
                  >
                    <div className="flex items-center justify-between">
                      <span className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded ${getStatusBadgeColors(event.status)}`}>
                        {event.status}
                      </span>
                    ...
                    <p className="text-xs text-slate-700 leading-relaxed font-sans">{event.notes}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-xs text-slate-400 text-center py-4 italic">No logs published yet.</p>
            )}
          </div>
        </div>
      </div>
